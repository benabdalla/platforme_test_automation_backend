Feature: Qualipro- Test fiche Action En tant que utilisateur,je souhaite vérifier les données de etat d'action par responsable
  ...
  @ReportingListeActionRealiseeParType
  Scenario Outline: reporting etat d'action par responsable
    Given Ouvrir le site QualiProWeb
   ...
   # And Exporter tracabilite
    And vérifier les données rapport liste des actions par type
    Then delete file
    Examples:
      | type_Rg   |
      | "Cloture" |
